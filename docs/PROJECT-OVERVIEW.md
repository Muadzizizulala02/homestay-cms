...
- **Booking flow + payment**: a guest can pick a unit, check real availability, create an actual `pending_payment` booking with server-computed pricing and the transactional double-booking-prevention guarantee, and be redirected to a real Billplz-hosted payment page. Billplz's webhook (signature-verified server-side, per `PAYMENT.md`) is what actually confirms a booking — nothing on the frontend can do that. If payment creation fails (no real Billplz credentials exist in this environment yet) the guest still gets a valid pending booking and an honest "we'll contact you" message rather than a broken flow.
